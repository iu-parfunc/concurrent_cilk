// TODO: clear uc cilk_worker stuff
...
int main_srch(position p, int depth);
int srch(search_state ss);
int ismemequal(char *p, char *q, int length);
hashentry get_hash(position p);
void set_hash(hashentry he, position p);
