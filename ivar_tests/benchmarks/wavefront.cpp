...
int main(int argc, char** argv) 
{
    ...
    matrix = new vector< ivar<tile*> >( dim*dim );

    // ----------------------------------------
    // This version works:
    vector< ivar<tile*>* > bar(dim*dim);
    ...
    if(! strcmp("parfor1",version) )
      // cilk_spawn
      traverse1();

    else if(! strcmp("parfor2",version) )
      // cilk_spawn
      traverse2();

    else if(! strcmp("cones",version) )
      // cilk_spawn
      spawn_cones();
     ...
     else printf("number need to be a power of two for dnc!\n");
    else {
