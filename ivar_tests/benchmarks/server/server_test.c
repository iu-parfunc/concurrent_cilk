...
// UNFINISHED -- the conversion of this test to libevent is not complete.
...
int main(int argc, char **argv) 
{
    ...
    if (sockfd < 0) {
        printf("Error openning socket: %d", errno);
        abort();
    }
    ...
    {
        printf("ERROR: on binding, errno %d\n", errno);
        abort();
    }
    listen(sockfd,50);
    ...
    // Seeing better initial performance with parallel spawning of multiple servers:
    // run_P(P,0, sockfd);
    run_server(0, sockfd);
#endif
    ...
    struct event evfifo;
    event_set(&evfifo, sockfd, EV_READ, wake_cilk_thread, &evfifo);
