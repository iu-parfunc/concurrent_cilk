...
/* find the maximal elelement (by absolute value) in the column
   from rows r1..r2 */
int column_max(int c, int r1, int r2)
{
     int j = 0, k;
     int r;
     Real max = -1.0;
     ...
     if (r2 - r1 < BLOCK_SIZE) {	/* can relax cilk_spawning here */
	  R = &A(r1, c);
	  for (r = r1; r <= r2;) {
	       if (fabs(*R) >= max) {
		    max = fabs(*R);
	  ...
	  r = (r2 + r1) / 2;
	  j = cilk_spawn column_max(c, r1, r);
	  k = cilk_spawn column_max(c, r + 1, r2);
	  cilk_sync;
...
void update_lower_rows(Real * Ri, int cb, int k, int t, int b)
{
     int l, m, j;
     Real a;
     Real *Rj;

     if (b - t < BLOCK_SIZE) {	/* can relax cilk_spawning here */
	  for (j = t; j <= b; j++) {
	  ...
	  m = (t + b) / 2;
	  cilk_spawn update_lower_rows(Ri, cb, k, t, m);
	  cilk_spawn update_lower_rows(Ri, cb, k, m + 1, b);
     }
}
...
void block_lup(int r, int c)
{
     ...
     for (k = 0; k < BLOCK_SIZE; k++) {
	  ...
	  j = cilk_spawn column_max(c + k, i, N - 1);
	  cilk_sync;
	  ...
	  /* update lower rows */
	  cilk_spawn update_lower_rows(Ri, c / BLOCK_SIZE, k, i + 1, N - 1);
	  cilk_sync;
	  Ri += BLOCK_SIZE;
     }
}
...
void block_schur(int ar, int ac, int br, int bc, int cr, int cc)
{
     int i, j, k;
     Real a;
     Real *RA, *R, *RB, *RC;
...
void schur(int ar, int ac, int br, int bc, int cr, int cc, int n,
		int m)
{
     if (m == BLOCK_SIZE) {
	  int s;
	  for (s = 0; s < n; s += m)
	       cilk_spawn block_schur(ar + s, ac, br + s, bc, cr, cc);
     } else {
	  m /= 2;
	  n /= 2;

	  cilk_spawn schur(ar, ac, br, bc, cr, cc, n, m);
	  cilk_spawn schur(ar, ac + m, br, bc + m, cr + m, cc + m, n, m);
	  cilk_spawn schur(ar + n, ac, br + n, bc + m, cr + m, cc, n, m);
	  cilk_spawn schur(ar + n, ac + m, br + n, bc, cr, cc + m, n, m);
	  cilk_sync;

	  cilk_spawn schur(ar, ac, br, bc + m, cr + m, cc, n, m);
	  cilk_spawn schur(ar, ac + m, br, bc, cr, cc + m, n, m);
	  cilk_spawn schur(ar + n, ac, br + n, bc, cr, cc, n, m);
	  cilk_spawn schur(ar + n, ac + m, br + n, bc + m, cr + m, cc + m, n,
		      m);
     }
}

/* solve: L*B=A (for B) */
void lower_solve(int lr, int lc, int ar, int ac, int m);
...
/* solve [L00  0 ] * [X0] = [A0]
         [L10 L11]   [X1]   [A1]  */
void lower_solve_aux(int lr, int lc, int ar, int ac, int m)
{
     cilk_spawn lower_solve(lr, lc, ar, ac, m);
     cilk_sync;

     cilk_spawn schur(ar + m, ac, lr + m, lc, ar, ac, m, m);
     cilk_sync;

     cilk_spawn lower_solve(lr + m, lc + m, ar + m, ac, m);
}
...
void lower_solve(int lr, int lc, int ar, int ac, int m)
{
     if (m == BLOCK_SIZE) {
	  ...
	  m /= 2;
	  cilk_spawn lower_solve_aux(lr, lc, ar, ac, m);
	  cilk_spawn lower_solve_aux(lr, lc, ar, ac + m, m);
     }
}

/* solve a LUP subproblem of size nxm */
void lup(int r, int c, int n, int m)
{
     if (m == BLOCK_SIZE) {
	  cilk_spawn block_lup(r, c);
     } else {

	  m /= 2;

	  cilk_spawn lup(r, c, n, m);
	  cilk_sync;

	  cilk_spawn lower_solve(r, c, r, c + m, m);
	  cilk_sync;

	  n -= m;

	  cilk_spawn schur(r + m, c + m, r + m, c, r, c + m, n, m);
	  cilk_sync;

	  cilk_spawn lup(r + m, c + m, n, m);
     }
}
