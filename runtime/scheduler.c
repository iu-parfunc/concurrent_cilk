...
static void random_steal(__cilkrts_worker *w)
{
  ...
  if(!can_steal_from(victim)) {
    ...
    if(victim->ivar && IVAR_READY(*victim->ivar) && !can_steal_from(victim)) {
      if( __cilkrts_mutex_trylock(victim, &victim->l->lock)) {
        //printf("restoring %d/%p off of a steal. running worker %d/%p\n", victim->self, victim, w->self, w);
...
#ifdef CILK_IVARS
      //replacement workers don't have a team, so 
      //running this would be pointless. 
      if (NULL != ff->sync_master && w->self != -1) {
#else
      ...
      __cilkrts_stack_frame *sf)
  {
    ...
    static void schedule_work(__cilkrts_worker *w)
    {
      ...
      if (NULL == ff) {
        ...
        if(w->ivar && IVAR_READY(*w->ivar)) {
          if( __cilkrts_mutex_trylock(w, &w->l->lock)) {
    ...
    static void __cilkrts_unbind_thread()
    {
...
#ifdef CILK_IVARS
          if (w->self == -1 && !w->is_replacement == 1) {
#else
            if (w->self == -1) {
#endif
      ...
      void __cilkrts_c_return_from_initial(__cilkrts_worker *w)
      {
        ...
        BEGIN_WITH_WORKER_LOCK_OPTIONAL(w) {
          ...
          printf("return form initial: ff %p ff->stack_self, %p current_stack_frame %p\n", ff, ff->stack_self, w->current_stack_frame);
#ifdef CILK_IVARS
          if(ff->stack_self) {
#endif
