...
void do_sync (__cilkrts_worker *w,
    full_frame *ff,
    __cilkrts_stack_frame *sf);
...
{
  cilk_dbg(SCHED, "[make_runnable] setting full frame %p in worker %d/%p\n",
      ff, w->self, w);
...
full_frame *pop_next_frame(__cilkrts_worker *w)
{
  ...
  //   cilk_dbg(1,"%d popping full frame. new frame %p\n",w->self, ff);
    ...
    full_frame *ff)
{
  ...
  void do_work(__cilkrts_worker *w, full_frame *ff)
  {
    ...
    cilk_dbg(1,"hey! my continuation got called. in longejumped after resume\n");
  ...
  static void __cilkrts_scheduler(__cilkrts_meta_sched do_sched, __cilkrts_worker *volatile w, void *volatile args)
  {
    ITT_SYNC_PREPARE(w);
